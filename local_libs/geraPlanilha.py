...
def preencheArquivoCSV(tabela,arquivo,driver,trip):
    ...
    for i in range(len(tabela)):
        ...
        ct.dictCSV["SPD_KMH"]=str('{:05f}'.format(velocidadeKMH)).replace('.',',')+';'
        ct.dictCSV["SPD_MPH"]=str('{:05f}'.format(velocidadeMPH)).replace('.',',')+';'
        ...
        if(i>0):    
            ...
            ct.dictCSV['ACEL_MS2']=str('{:05f}'.format(aceleracaoMPS)).replace('.',',')+';'            
...
def geraSemVideoCSV(viagensBack,condutor,diretorioGPSConcatenado):
    ...
    if(len(tabela)>1):
        with open(semVideoCSV, 'w') as arquivoGPS:
            preencheArquivoCSV(tabela,arquivoGPS,condutor,viagensBack[len(viagensBack)-1].index+1)
        arquivoGPS.close()   
